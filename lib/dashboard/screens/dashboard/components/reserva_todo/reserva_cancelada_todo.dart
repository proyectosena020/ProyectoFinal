  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            texts.reservas_todo.reservas_canceladas.title,
            style: Theme.of(context).textTheme.titleMedium,
          ),
          FutureBuilder(
              future: getReservas(), // llama todas las reservas
              builder:
                  (context, AsyncSnapshot<List<ReservaModel>> reservaCancel) {
                // Mientras carga la información
                if (reservaCancel.connectionState == ConnectionState.waiting) {
                  return const Center(
                ...
                if (reservaCancel.data != null) {
                  for (var r = 0; r < reservaCancel.data!.length; r++) {
                    // Verifica si hay reservas canceladas
                    if (reservaCancel.data![r].estado == "Cancelado") {
...
DataRow ReservaCtDataRow(ReservaModel reservaCInfo, BuildContext context) {
    ...
    cells: [
      ...
      DataCell(ElevatedButton(
        onPressed: () {
          ...
          _modalReserva(context, reservaCInfo);
        },
          ...
          builder: (context, AsyncSnapshot<List<UsuariosModel>> usuario1) {
            ...
            for (var v = 0; v < usuario1.data!.length; v++) {
              if (reservaCInfo.usuario == usuario1.data![v].id) {
                ...
                estado = reservaCInfo.estado;
                lugar = reservaCInfo.sitio.titulo;
              }
            }
            return IconButton(
                onPressed: () {
                  ...
                  String pedido = "";
                  pedido = "$pedido "
                      "${texts.reservas_todo.reservas_canceladas.summery.text1} "
                      "${texts.reservas_todo.reservas_canceladas.summery.nameSite(name: reservaCInfo.sitio.titulo)}\n"
                      "${texts.reservas_todo.reservas_canceladas.summery.location(name: reservaCInfo.sitio.lugar)} \n"
                      "${texts.reservas_todo.reservas_canceladas.summery.checkInDate(name: reservaCInfo.fechaEntrada)} \n"
                      "${texts.reservas_todo.reservas_canceladas.summery.checkOut(name: reservaCInfo.fechaSalida)} \n"
                      "${texts.reservas_todo.reservas_canceladas.summery.total(name: reservaCInfo.numHuespedes)} \n"
                      "${texts.reservas_todo.reservas_canceladas.summery.totalPrice(name: reservaCInfo.precioFinalFormatted)}";
