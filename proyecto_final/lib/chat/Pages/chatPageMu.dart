...
class ChatPageMu extends StatefulWidget {
  const ChatPageMu({super.key});

  @override
  State<ChatPageMu> createState() => _ChatPageMuState();
}

class _ChatPageMuState extends State<ChatPageMu> {
  List<UsuariosModel> misHuespedes = []; // Lista de huéspedes relacionados
        ...
        builder: (context, AsyncSnapshot<List<ReservaModel>> reserva) {
          ...
          return FutureBuilder(
            ...
            builder: (context, AsyncSnapshot<List<UsuariosModel>> usuario) {
              ...
              Set<UsuariosModel> huespedesSet = {};

              // Itera sobre las reservas y usuarios para encontrar los huéspedes relacionados
                ...
                for (var r = 0; r < reserva.data!.length; r++) {
                  if (reserva.data![r].estado == "Activo") {
                    if (reserva.data![r].usuario == usuario.data![u].id &&
                        reserva.data![r].sitio.usuario == miusuario.id) {
                      huespedesSet.add(usuario.data![u]);
                    }
                  }
                }
              }
              ...
              misHuespedes = huespedesSet.toList();

              // Construye una lista de tarjetas personalizadas para cada huésped
              return ListView.builder(
                itemCount: misHuespedes.length,
                itemBuilder: (context, index) => CustomCard(
                  usuario: misHuespedes[index],
